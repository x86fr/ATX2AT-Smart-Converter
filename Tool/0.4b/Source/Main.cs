...
namespace ATX2AT_Configuration_Tool
{
    ...
    {
        // Min FW 1.21
        const int Min_FW_Rev_Major = 1;
        const int Min_FW_Rev_Minor = 21;
        ...
        private void GetEEPROMInfo(byte[] BytesCMD)
        {
            ...
            // If FW < Min_FW_Rev, display warning
            if (BytesCMD[4] < Min_FW_Rev_Major || BytesCMD[5] < Min_FW_Rev_Minor)
            {
                timer1.Enabled = false;
                MessageBox.Show("The Firmware is out-of-date. Please upgrade!", "FW Outdated!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                timer1.Enabled = true;
            }
            ...
            } else { cBox_ssaver.SelectedIndex = cBox_ssaver.Items.Count - 1; }
            ...
            // Set Power Switch Type
            cBox_Pwr_Type.SelectedIndex = BytesCMD[8] & 1;
            Btn_OnOff.Enabled = cBox_Pwr_Type.SelectedIndex == 0;
        ...
        private void Btn_Save_PwrBtnType_Click(object sender, EventArgs e)
        {
            byte[] cmd = new byte[] { 0x56, 0x06, 0xB1, 0x05, 0x00, 0x00 };

            cmd[4] = (byte)cBox_Pwr_Type.SelectedIndex;
        ...
        private void Btn_Fw_Upd_Click(object sender, EventArgs e)
        {
            timer1.Enabled = false;
            if (ProcessCOM.IsConnected) { ProcessCOM.CloseConnection(); }

            FW_Upd f2 = new FW_Upd();
            f2.ShowDialog();
